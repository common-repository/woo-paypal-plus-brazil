jQuery( function ( $ ) {
	...
	var ppb_checkout = {
		...
		init: function () {
			...
			this.$checkout_form.on( 'change', '#billing_first_name, #billing_last_name, #billing_cpf, #billing_cnpj, #billing_email, #billing_address, #billing_number, #billing_address2, #billing_neighborhood, #billing_city, #billing_state, #billing_country, #billing_phone', this.trigger_update_checkout );
		...
		updated_checkout: function () {
			...
			if ( info.length ) {
				...
				ppb_checkout.ppp = PAYPAL.apps.PPP( {
					...
					payerPhone: info_json.billing_phone,
					language: 'pt_BR',
					country: 'BR',
					iframeHeight: '400',
